...
function getPositiveArrayValues(arr) {
  ...
  const arrayVal = Array.from(iterator);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const flatVal = arr.flat();
  const squared = flatVal.map((elem) => elem * elem);
  const filt = squared.filter((element) => element > threshold);
  return filt;
}
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const getLastElements = arr.slice(-n);
  const getSum = getLastElements.reduceRight(
    (accumulator, currentValue) => accumulator + currentValue,
    0
  );
  getLastElements.forEach((element) => console.log(element));
  return getSum;
}
...
function wordLengths(arr) {
  ...
  const lengths = arr.map((fruits) => fruits.length);
...
function capitalizeWords(arr) {
  ...
  const capitaliz = arr.map((fruits) => {
...
function findLastNegativeNumberIndex(arr) {
  ...
  const isLargeNumber = arr.findLastIndex((num) => num < 0);
